...
#          links x265-static, ends up carrying all three depths.
#
...
configure() {
    ...
    # needs do not exist yet: CMake turns the bare names in EXTRA_LIB into
    # -lx265_main10/-lx265_main12 and the linker resolves them through
    # -DEXTRA_LINK_FLAGS=-L. when the 8-bit target is built (build()).
...
build() {
    ...
    # EXTRA_LIB entries reach the linker as -lx265_main10 and -lx265_main12,
    # so the archives must carry those names inside the 8-bit build directory
    # and be reachable through -L. multilib.sh symlinks them; a copy is used
    # here so the same recipe works when the trees live on different mounts.
...
install() {
    ...
    if ! grep -qa '8bit+10bit+12bit' "$dest/$name$exe"; then
